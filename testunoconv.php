...
$PAGE->set_url(new moodle_url('/mod/assign/feedback/editpdfplus/testunoconv.php'));
$PAGE->set_context(context_system::instance());

require_login();
require_capability('moodle/site:config', context_system::instance());

$strheading = get_string('test_unoconv', 'assignfeedback_editpdfplus');
...
$PAGE->navbar->add(get_string('pluginname', 'assignfeedback_editpdfplus'),
        new moodle_url('/admin/settings.php', array('section' => 'assignfeedback_editpdfplus')));
...
switch ($result->status) {
    case file_storage::UNOCONVPATH_OK:
        $msg = $OUTPUT->notification(get_string('test_unoconvok', 'assignfeedback_editpdfplus'), 'success');
        $pdflink = new moodle_url($PAGE->url, array('sendpdf' => 1, 'sesskey' => sesskey()));
        $msg .= html_writer::link($pdflink, get_string('test_unoconvdownload', 'assignfeedback_editpdfplus'));
    ...
    default:
        $msg = $OUTPUT->notification(get_string("test_unoconv{$result->status}", 'assignfeedback_editpdfplus'), 'warning');
        break;
}
